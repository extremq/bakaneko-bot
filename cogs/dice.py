...
class DiceCog(commands.Cog):
    ...
    @discord.app_commands.command(
        name="dice",
        description="サイコロを振ります。例: 1d6, 2d20, d10"
    )
    async def dice_command(self, interaction: discord.Interaction, dice_list: str):
        dice_pattern = re.compile(r'(\d*)d(\d+)')
        matches = dice_pattern.findall(dice_list.lower())

        if not matches:
            await interaction.response.send_message(
                "無効なダイス形式です。`1d6`, `2d20`, `d10` のような形式で入力してください。"
            )
            return

        if len(matches) > 10:
            await interaction.response.send_message(
                "一度に振れるダイスの種類は10個までです。"
            )
            return
        ...
        for num, faces in matches:
            ...
            if num > 10:
                await interaction.response.send_message(
                    f"{num}d{faces}: 1回のコマンドで10回以上振ることはできません。\n"
                )
                return
